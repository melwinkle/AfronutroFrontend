...
import { fetchCuisineType } from "../../redux/slices/dataSlice";


const CuisinePreference = ({ nextStep, prevStep, updateFormData,data }) => {
  ...
  const filteredPreferences = cuisines.filter((preference) =>
    preference.display_name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleNext = () => {
    updateFormData({ cuisine_preference: selectedPreferences });
    nextStep();
  };

  return (
    <div>
      <h2 className="text-2xl font-bold mb-1 text-center">What are your cuisine preferences?</h2>
